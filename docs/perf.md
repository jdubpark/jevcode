...
| Jev Pass A batch (≤8 units) | ≤3s p95 | not separately measured (no keyed environment; degrade path is synchronous, sub-ms per unit) | PASS (degrade path); unmeasured for live TypeSafe (typesafe-spike §4) |
...
| Initial repo index (10k files) | ≤60s background | not applicable in replay mode (evidence disabled); tree-sitter index path not exercised by the soak | NOT MEASURED |
...
| Event store growth cap | 1M events/session, then archive | soak stored 75,181 events for 9,993 records (7.5x amplification: facts + projections + jev logs + snapshots); cap enforced by schema/bound checks | PASS |

## Soak (SPEC §16 acceptance criterion, T9-2)

`node scripts/soak.mjs` — 9,993 generated records (formatting/file-noise bursts +
periodic real changes + tests + decisions) replayed through the real pipeline
(degrade router, storage-backed, SQLite in tmpdir):

- No crash; session reached `completed`; completion surface emitted.
...
1. `PipelineCoordinator.rebuild()` had an O(units²) signature-cleanup loop
   (`[...unitSignatures].some(unit => ...)` per signature); replaced with a
   Set lookup (`packages/semantic-core/src/coordinator.ts`).
...
`git_hunk` — replayed through the real coordinator; graph projection measured in
isolation with one unit per file):
...
- Trailing-edge rebuild debounce in `PipelineCoordinator`: at most one full
  rebuild per flush burst (25ms trailing idle; `flush()` still rebuilds
  synchronously). Trickle bursts collapse from one rebuild per 500ms window
  to one per burst.
- `projectGraph`: O(F²) fact→unit ownership scans replaced with a fact-reference
  ...
  cache; per-fact `units.find` replaced with a unit-id Map.
...
Remaining known cost: a rebuild still re-clusters the full session — bounded by
the debounce to one rebuild per burst instead of one per batch window; full
incremental clustering is out of scope (documented in SPEC §19).
