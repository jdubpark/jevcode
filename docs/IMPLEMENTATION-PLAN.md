...
Key split: **T5 is split by model-dependence** — guardrails and degrade mode (T5-3, T5-5) are pure and parallelizable immediately after T0, while SDK integration and prompt assembly (T5-1, T5-2) wait for the spike and T4 outputs.
...
- T9-1 E2E demo scenario (Playwright/Electron): rate-limit task, mocked agent, decision loop, completion review (2.5ed) — **DONE (headless form):** `apps/desktop/src/main/pipeline/demo-e2e.test.ts` asserts the full PRD §58 sequence through the real pipeline; live runbook in `docs/demo.md`
- T9-2 Soak: 10k-event replay, SurfaceManager stability assertions (1ed) — **DONE:** `scripts/soak.mjs` (9,993 records, invariants held, timings in `docs/perf.md`); fixed two quadratic rebuild paths found by the soak
...
- T9-5 Bugfix buffer, docs, PRD §59 acceptance walkthrough (2.5ed) — **DONE:** `docs/acceptance.md` maps all 17 criteria to evidence; completion summary surface (acceptance #14) implemented; thread-id resume wiring completed; evals green (MAE 0.048, representation set-acc 1.0)
...
Note: Lane C owns M0 (week 4), M4–M7 integration; Lane B owns M2/M3 and eval gates; Lane A owns M1. M4–M7 are joint A+C efforts (agent decision loop spans T2 + T6).
...
≈ 40ed on Lane B/C boundary. At 5ed/week this is ~8 weeks; with M0/M1 work fully parallelized, the **minimum viable schedule is 8 weeks, planned 10 with buffer**. To compress: add a 4th engineer to T3/T4 (split collectors vs. clustering), or cut P1 components (saves 7ed on Lane C, which is not on the critical path — only do this for time, not scope).
...
- `SPEC.md` updated where implementation forced a contract change; no silent drift.
