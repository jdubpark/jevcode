...
- Package: `@typesafe-ai/sdk` (v0.6.0 installed in `packages/jev-router`). Node >= 20 required. The repo uses Node >= 22. ESM + CJS + TS declarations. We consume the ESM build.
...
- **Score**: answer = `{ type: "score", score, legend, confidence, probabilities }`. `score` is a probability-weighted position on the level-number line (0..top-level, and can be fractional). `probabilities` is the distribution over levels. Normalization to 0..1: divide by `len(criteria) - 1` (docs show exactly this).
...
- **Batching**: any number of questions can be sent over one state in one `systemOne` call. They are evaluated in parallel and one answer does not inform another. Adding questions "barely changes the response time" and costs only question tokens (docs. The parallel-questions cookbook reports ~10x faster than per-question calls). Request budget ~32k tokens shared by state + questions. Question IDs are for code only and are not sent to the model. The full question must live in `instructions`. State fields are referenced from instructions with backticked paths (e.g. `` `units[0].files` ``).
...
Prompt policy: the Jevcode catalog, triad definitions, mental-model-delta definition, and PRD section 34 examples are embedded verbatim as `state.system_policy`. TypeSafe has no system-prompt field, since state is the context channel.

Input state per unit (SPEC 8.2): title, intent, category hint, file set (<= 40), symbol names (<= 60), and diff stats. Plus formatting/lockfile flags, destructive commands, and security/schema paths. Plus public exports, behavior/interfaces hints, dependency deltas, test outcomes, and decision presence. The task prompt is truncated to 4096 chars.
...
- **Run-verified (offline)**: question composition, answer->JevResult mapping, Score/Choice/Noul handling, batched request assembly, retry-once-then-deterministic-defaults, health/selection, and the full guardrail + policy pipeline. Thirteen unit tests exercise these paths against an injected fake transport (`TypeSafeTransport`). No real network calls run in the test suite. The real `SdkTransport` (thin wrapper over the SDK) is construction-verified by typecheck only.
...
- jev-router keeps a local copy of the destructive-command pattern list (`src/patterns.ts`). The evidence-engine track is building its own classifier concurrently (T3-7). Consolidate to one list when both tracks land.
...
- Available models: `jev-latest` (default), `jev-preview`. Both measured on the eval suite. Performance is equivalent.
...
- Scores return continuous values between integer levels. The `score/(levelCount-1)` mapping is confirmed correct.
